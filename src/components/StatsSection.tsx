...
import { Music, Tv, Shield, Star, Users, Award, Globe, Zap } from "lucide-react";
...
const StatsSection = () => {
  const { stats, loading } = useSiteContent();
  ...
  const displayStats = stats.length > 0 ? stats : defaultStats.map(s => ({ ...s, icon: s.icon.name }));
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {loading
            ...
            : (stats.length > 0 ? stats : defaultStats).map((stat, i) => {
                const IconComponent = typeof stat.icon === 'string' ? (iconMap[stat.icon] || Music) : stat.icon;
                return (
                  <motion.div
